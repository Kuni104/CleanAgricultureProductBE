...
namespace CleanAgricultureProductBE.Services.Cart
{
    ...
    {
        public Task<ResultStatusWithData<AddToCartResponseDto>> AddToCart(string userId, Guid productId, CartRequestDto request);
        public Task<CartItemResponseWithPaginationDto> GetCartItem(string accountEmail, int? page, int? size, string? keyword);
        public Task<ResultStatusWithData<UpdateCartResponseDto>> UpdateCartItemQuantity(string accountEmail, Guid productId, CartRequestDto request);
